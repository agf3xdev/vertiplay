...
import { Mail, Apple, ChevronLeft, Loader2 } from "lucide-react";
...
function AuthInner() {
  ...
  // Quando rodando dentro do app nativo (Capacitor), OAuth web do Google
  // não funciona porque o WebView perde cookies no redirect. Escondemos
  // Google e forçamos email OTP. Detecção via user-agent (Capacitor injeta).
  const [isNativeApp, setIsNativeApp] = useState(false);
  useEffect(() => {
    if (typeof window === "undefined") return;
    const ua = navigator.userAgent || "";
    const inCapacitor = /capacitor|vertiplay/i.test(ua) || (window as any).Capacitor != null;
    setIsNativeApp(inCapacitor);
  }, []);
